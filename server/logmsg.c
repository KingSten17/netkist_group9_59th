...
int main() {
    ...
    // 测试日志输出
    D_log(INFO,"Application started. IP: %s, Port: %s", cfg.ip, cfg.port);
    D_log(DEBUG, "Debug message. User: %s", cfg.mysql_user);
    D_log(WARNING, "Warning message. Password: %s", cfg.mysql_password);
    D_log(ERROR, "Error message. Log type: %s" ,"transFile error");
